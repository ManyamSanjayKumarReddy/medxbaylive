...
const ManageAppointments = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const response = await fetch('https://medxbay-deploy-1-431103.uc.r.appspot.com/doctor/bookings', {
          credentials: 'include',
          headers: {
  ...
  const confirmAndUpdateStatus = async (id) => {
    ...
    if (window.confirm(`Are you sure you want to change the status to ${newStatus}?`)) {
      try {
        const response = await fetch(`https://medxbay-deploy-1-431103.uc.r.appspot.com/doctor/bookings/${id}`, {
          method: 'POST',
          credentials: 'include',
          headers: {
